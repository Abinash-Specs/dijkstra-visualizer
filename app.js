(function () {
  ...
  const pathSelectedText = document.getElementById("path-selected-text");
  ...
  function edgeOnPathByKeys(a, b, pathEdgeKeys) {
    if (!pathEdgeKeys || pathEdgeKeys.size === 0) return false;
    return pathEdgeKeys.has(edgeKeyFromPair(a, b));
  }
  ...
  function cloneFrame(f) {
    ...
    return {
      ...
      pathEdgeKeys: f.pathEdgeKeys ? new Set(f.pathEdgeKeys) : null,
      note: f.note,
    };
  }
  ...
  function edgePathStrength(fromF, toF, a, b, te) {
    if (!fromF || !toF || fromF === toF) {
      return edgeOnPathByKeys(a, b, toF?.pathEdgeKeys) ? 1 : 0;
    }
    const o0 = edgeOnPathByKeys(a, b, fromF.pathEdgeKeys);
    const o1 = edgeOnPathByKeys(a, b, toF.pathEdgeKeys);
    if (o0 && o1) return 1;
    if (!o0 && !o1) return 0;
    return o1 ? te : 1 - te;
  }
  ...
  function formatPathLabels(pathIds) {
    return pathIds.map((id) => labelFor(id)).join(" → ");
  }

  function patchLastFramePath(pathIds) {
    const last = animationFrames[animationFrames.length - 1];
    if (!last) return;
    const { path, pathEdgeKeys } = buildPathHighlight(pathIds);
    last.path = path;
    last.pathEdgeKeys = pathEdgeKeys;
  }

  function refreshPathPickerUI() {
    if (!pathSection || !pathSelectedText || !pathButtonsEl) return;
    pathButtonsEl.replaceChildren();
    pathButtonsEl.hidden = false;
    const len = animationFrames.length;
    const onFinalFrame = len > 0 && frameIndex === len - 1;
    const playbackIdle = !playing || animPaused;
    const atResult =
      shortestPathsAll != null && onFinalFrame && playbackIdle;
    if (!atResult) {
      pathSection.hidden = true;
      return;
    }
    pathSection.hidden = false;
    if (shortestPathsAll.length === 0) {
      pathSelectedText.textContent = "No path to destination.";
      pathButtonsEl.hidden = true;
      return;
    }
    if (shortestPathsAll.length === 1) {
      pathSelectedText.textContent = "Path: " + formatPathLabels(shortestPathsAll[0]);
      pathButtonsEl.hidden = true;
      return;
    }
    pathButtonsEl.hidden = false;
    for (let i = 0; i < shortestPathsAll.length; i++) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "btn btn-secondary btn-path";
      btn.textContent = "Path " + (i + 1);
      const active = i === selectedPathIndex;
      btn.setAttribute("aria-pressed", active ? "true" : "false");
      if (active) btn.classList.add("active");
      btn.addEventListener("click", () => selectShortestPath(i));
      pathButtonsEl.appendChild(btn);
    }
    pathSelectedText.textContent =
      "Selected Path: " + formatPathLabels(shortestPathsAll[selectedPathIndex]);
  }

  function selectShortestPath(i) {
    if (!shortestPathsAll || shortestPathsAll.length <= 1) return;
    if (i < 0 || i >= shortestPathsAll.length) return;
    if (!animationFrames.length || frameIndex !== animationFrames.length - 1) return;
    const fromSnap = cloneFrame(currentFrameState());
    selectedPathIndex = i;
    patchLastFramePath(shortestPathsAll[i]);
    beginBlend(fromSnap, cloneFrame(animationFrames[frameIndex]));
    refreshPathPickerUI();
    draw();
  }
  ...
  function buildShortestPathParents(adj, dist, nodeIds) {
    const INF = Infinity;
    const parents = new Map();
    for (const id of nodeIds) parents.set(id, []);
    for (const u of nodeIds) {
      const du = dist.get(u);
      if (!Number.isFinite(du) || du === INF) continue;
      for (const { to: v, w } of adj.get(u) || []) {
        const dv = dist.get(v);
        if (Number.isFinite(dv) && du + w === dv) {
    ...
    for (const id of nodeIds) {
      ...
      parents.set(id, [...new Set(arr)].sort((a, b) => a - b));
    }
    return parents;
  }

  function enumerateShortestPaths(srcId, destId, dist, parents) {
    const INF = Infinity;
    if (dist.get(destId) === INF) return [];
    const memo = new Map();
    function pathsTo(node) {
    ...
    return pathsTo(destId);
  }

  function buildPathHighlight(pathIds) {
    if (!pathIds || pathIds.length === 0) {
      return { path: new Set(), pathEdgeKeys: new Set() };
    }
    const path = new Set(pathIds);
    const pathEdgeKeys = new Set();
    for (let i = 0; i < pathIds.length - 1; i++) {
      pathEdgeKeys.add(edgeKeyFromPair(pathIds[i], pathIds[i + 1]));
    }
    return { path, pathEdgeKeys };
  }

  function computeDijkstraSteps(srcId, destId) {
    const ids = nodes.map((n) => n.id);
    if (ids.length === 0) {
      return { frames: [], prev: new Map(), finalDist: new Map(), allPaths: [] };
    }
    ...
    while (pq.size > 0) {
      ...
      for (const { to, w } of adj.get(u) || []) {
        ...
        if (nd < dist.get(to)) {
          ...
          multiPrev.set(to, [u]);
          pq.push([nd, to]);
        } else if (nd === dist.get(to)) {
          const mp = multiPrev.get(to);
          if (mp && !mp.includes(u)) mp.push(u);
        }
      }
    ...
    const parentsDAG = buildShortestPathParents(adj, dist, ids);
    const allPaths =
      destId != null ? enumerateShortestPaths(srcId, destId, dist, parentsDAG) : [];
    const primary = allPaths.length ? allPaths[0] : [];
    const { path: pathSet, pathEdgeKeys } = buildPathHighlight(primary);

    let doneNote;
    if (destId == null || dist.get(destId) === INF) {
      doneNote = "Done — No path to destination (unreachable).";
    } else if (allPaths.length <= 1) {
      doneNote = "Done — Shortest path highlighted (green).";
    } else {
      doneNote =
        "Done — " +
        allPaths.length +
        " shortest paths (same total weight). Use Show path to switch between them.";
    }
    ...
    return { frames, prev, finalDist: dist, allPaths };
  }
  ...
  function startAnimation() {
    ...
    const { frames, allPaths } = computeDijkstraSteps(sourceId, destId);
    shortestPathsAll = allPaths;
    selectedPathIndex = 0;
  ...
  function edgeKey(e) {
    return edgeKeyFromPair(e.from, e.to);
  }
